...
1. menampilkan landing page, search, detail destinasi, compare, profile, dan favorite;
...
- Untuk protected route, middleware juga membaca cookie/token yang relevan.
...
- menampilkan carousel destinasi rekomendasi;
- menjelaskan sinyal AI, sentimen, dan topik secara visual.

Alur:
1. Page public mengambil data rekomendasi dari backend.
2. Data diteruskan ke komponen home.
3. CTA search mengarahkan user ke `/search?q=...`.
...
4. User bisa membuka galeri, menambah favorite, membaca topik, atau mengirim review.
...
- `TopicsClient`: mengelola taxonomy topic, group, rename, visibility, dan AI naming.
- `AdminTopicService`: service API manajemen topic dan topic group.

Alur:
1. `TopicsClient` mengambil topics dan topic groups.
2. Data dihitung menjadi metrics, chart, table, dan queue action.
3. Admin bisa rename, delete, update settings, atau menjalankan AI naming.
...
Alur:
1. Admin memilih destinasi dan jumlah review.
...
- menampilkan hasil ringkas proses NLP.

Komentar penting:
- `NlpProcessingClient`: mengelola upload file review dan proses NLP manual.
- `AdminNlpService`: service API upload file dan proses NLP.

Alur:
1. Admin memilih file dan destination ID.
2. Client validasi file di browser.
3. Service mengirim multipart form ke backend.
4. Backend memanggil service Python Model.
5. Result ringkas ditampilkan di UI.
...
- `nlp.service.ts`: upload file NLP.
- `topic.service.ts`: topic, group, rename, visibility.
...
- NLP upload: `NlpProcessingClient` -> backend admin NLP -> Model pipeline -> database -> summary.
...
| `web/src/middleware.ts` | Route protection | Mengarahkan user berdasarkan cookie auth untuk route public/private/admin. | `middleware` `middleware.ts:9`, `config` `middleware.ts:60` |
...
| `web/src/components/home/HeroSection.tsx` | Tampilan home | Hero utama landing dengan CTA pencarian dan visual brand. | `HeroSection` `HeroSection.tsx:39` |
| `web/src/components/home/TrendingCarousel.tsx` | Tampilan rekomendasi | Carousel destinasi dengan card image dan metric sentimen. | `TrendingCarousel` `TrendingCarousel.tsx:45` |
...
| `web/src/components/destinations/DestinationDetailClient.tsx` | Detail interaktif | Mengatur hero, favorite, review, topic, gallery, video, dan navigasi section. | `DestinationDetailClient` `DestinationDetailClient.tsx:179`, `ReviewCard` `DestinationDetailClient.tsx:802` |
...
| `web/src/components/admin/topics/TopicsClient.tsx` | Topic admin | Mengatur daftar topic, rename, delete, filter group, dan visibility. | Container topic admin |
| `web/src/components/admin/topics/topics-client.panels.tsx` | Panel topic admin | Hero, command panel, cloud, naming debt, checklist, dan badge status. | `NamingDebtPanel` `topics-client.panels.tsx:205`, `TopicQualityChecklist` `topics-client.panels.tsx:306` |
| `web/src/components/admin/topics/TopicAnalyticsCharts.tsx` | Chart topic admin | Visual coverage dan distribusi topic. | `TopicCoverageParetoChart` `TopicAnalyticsCharts.tsx:50` |
...
| `web/src/components/admin/nlp/NlpProcessingClient.tsx` | NLP admin | Upload file review, pilih destinasi, proses NLP, dan tampilkan hasil. | `NlpProcessingClient` `NlpProcessingClient.tsx:47`, `NlpResultWorkspace` `NlpProcessingClient.tsx:319` |
...
| `web/src/components/admin/compare/CompareClient.tsx` | Compare admin | Analisis single/dua destinasi, chart, risiko sentimen, dan rekomendasi admin. | `CompareClient` `CompareClient.tsx:114` |
...
| `web/src/components/ui/native-select.tsx` | UI primitive | Select custom untuk filter/form. | `NativeSelect` `native-select.tsx:28` |
