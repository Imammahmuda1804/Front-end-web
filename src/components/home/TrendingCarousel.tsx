...
function destinationImage(destination?: Destination) {
  return destination?.thumbnailUrl
    ? getImageUrl(destination.thumbnailUrl)
    : '/images/auth-bg.jpg';
}
...
export function TrendingCarousel({ destinations }: TrendingCarouselProps) {
  const items = useMemo(() => destinations.slice(0, 7), [destinations]);
      ...
      <div className="mx-auto max-w-7xl px-6 md:px-12">
        <header className="mb-10 grid gap-5 border-b border-slate-300/70 pb-7 lg:grid-cols-[minmax(0,1fr)_28rem] lg:items-end">
          <div>
            ...
            <h2 id="destination-rail-title" className="mt-3 max-w-4xl text-4xl font-extrabold leading-[1.02] tracking-tight text-slate-950 md:text-6xl">
        ...
        <div className="grid gap-7 lg:grid-cols-[minmax(0,1.55fr)_minmax(19rem,0.65fr)] lg:gap-10">
          <article className="group relative min-h-[34rem] overflow-hidden rounded-xl bg-slate-950 md:min-h-[42rem]">
            <Image
              key={active.id}
              ...
              {items.map((destination, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <ArrowRight className={`h-4 w-4 transition-transform duration-150 ${selected ? 'translate-x-0 text-primary' : '-translate-x-1 text-slate-300'}`} />
                  </button>
                );
              })}
            </div>
