  ...
  return (
    <Card className="overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white py-0 shadow-sm">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 p-6">
        ...
        <div className="flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-black text-slate-500">
        ...
        {allActivities.length > 0 ? (
          <div className="space-y-3">
            {allActivities.map((item) => (
              <div key={item.id} className="flex flex-col justify-between gap-4 rounded-2xl border border-transparent bg-slate-50 p-3 transition-colors hover:border-orange-100 hover:bg-orange-50/60 sm:flex-row sm:items-center">
                <div className="flex min-w-0 items-center gap-4">
                  <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border ${getIconBg(item.type, item.status)}`}>
